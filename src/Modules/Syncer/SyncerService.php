...
class SyncerService
{
    ...
    public function __construct(private readonly FileManagerService     $fileManagerService,
                                ...
                                private readonly TripUserRoleRepository $tripUserRoleRepository,
                                private readonly SkillRepository        $skillRepository,
                                private readonly MasterTypeRepository   $masterTypeRepository)
    {
    }

    private function saveLocations(?string $locationType): void
    {
    ...
    private function saveSkills(): void
    {
        ...
        foreach ($this->skills as $skill) {
            $dbSkill = $this->skillRepository->findOneBy(["objID" => $skill["objID"]]);

            if (!isset($dbSkill) || $dbSkill->getSyncDate()->getTimestamp() < ((int)$skill["syncStatusDateTime"] ?? 0)) {
                $dbSkill = $dbSkill ?? new Skill();
                //dump($dbSkill);
                $dbSkill->setObjID($skill["objID"])
                    ->setSyncDate(\DateTime::createFromFormat('U', (int)$skill["syncStatusDateTime"]))
                    ->setName($skill["name"])
                    ->setTags($skill['tags'] ?? null)
                    ->setSkillDescription($skill['skillDescription'] ?? null)
                    ->setInternationalName($skill['internationalName'] ?? null)
                    ->setInternationalDescription($skill['internationalDescription'] ?? null)
                    ->setOwnerID($skill['ownerID'] ?? '1');
            }
            //dump($dbCheckList);
            $this->skillRepository->save($dbSkill);
        }
    }
        ...
        foreach ($objects as $object) {
            switch ($object['type']) {
                ...
                case 'Comment':
                    ...
                    break;
                case 'Skill':
                    $this->skills[$object['object']['objID']] = $object['object'];
                    break;
                case 'Syncer':
    ...
    public function getSyncResponse(string $username):array {
        ...
        $skillsResponse = $this->mapToSyncResponseListItems($this->skillRepository->getObjectsForSync($lastSuccessfulSyncDate),
            $this->skills,
            'Skill'
        );

        $masterTypesResponse = $this->mapToSyncResponseListItems($this->masterTypeRepository->getObjectsForSync($lastSuccessfulSyncDate),
            $this->skills,
            'MasterType'
        );
        ...
        //dump ($responseList);
        $responseList = array_merge($locationsResponse,$tripsResponse,$milestonesResponse,$commentsResponse,$checklistsResponse,$skillsResponse,$masterTypesResponse);
        $responseList[] = $syncer;
        return $responseList;
    }
